...
function createP12RSA_DOMAIN_SUB_CA_TRUST_Cert(uid, text, req, res, next) {
	try {
		  // generate a keypair
		  console.log('Generating 4096-bit SUB_CA_DOMAIN - key-pair...');
		  ...
		  console.log('Creating SUB_CA_DOMAIN certificate...');
...
//		    dataEncipherment: true
		    ...
		    altNames: [{
		      type: 6, // URI
		      value: 'https://www.prathamesh-phadnis.com/subca-domain/admin'
		    }]
		  }]);
		// read ca root private key  
		  ...
		  console.log('Certificate SUB_CA_DOMAIN created.');
		  ...
		  fs.writeFile('./rsa_sub_root/RSA_SUB_CA_DOMAINcertp12b64.p12', newPkcs12Der, {encoding: 'binary'} , function (err, file) {
				if (err) throw err;
		...
		function loadPkcs12(pkcs12Der, password, caStore) {
		  ...
		  for(var sci = 0; sci < pkcs12.safeContents.length; ++sci) {
		    ...
		    for(var sbi = 0; sbi < safeContents.safeBags.length; ++sbi) {
		      ...
		      if(safeBag.type === forge.pki.oids.pkcs8ShroudedKeyBag) {
		        ...
		        console.log('found SUB_CA_DOMAIN certificate');
		  ...
		  for(var localKeyId in map) {
		    ...
		    if(entry.privateKey) {
		      ...
		      fs.writeFile('./rsa_sub_root/RSA_SUB_CA_DOMAINprvKey.pem', privateKeyP12Pem, function (err, file) {
					if (err) throw err;
					console.log('Saved  SA_SUB_CA_DOMAINprvKey.pem file!');
				});
		    ...
		    if(entry.certChain.length > 0) {
		      ...
		      for(var i = 0; i < certChain.length; ++i) {
		        ...
		        fs.writeFile('./rsa_sub_root/RSA_SUB_CA_DOMAINCert.pem', certP12Pem, function (err, file) {
					if (err) throw err;
					console.log('Saved  RSA_SUB_CA_DOMAINCert.pem file!');
				});
		      }
...
exports.createP12RSA_DOMAIN_SUB_CA_TRUST_Cert = createP12RSA_DOMAIN_SUB_CA_TRUST_Cert;
createP12RSA_DOMAIN_SUB_CA_TRUST_Cert("Ajeet Phadnis", '', '', '', '');