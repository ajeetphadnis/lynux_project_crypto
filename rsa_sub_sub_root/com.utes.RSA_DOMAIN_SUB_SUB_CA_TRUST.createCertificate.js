...
function createP12RSA_DOMAIN_SUB_SUB_CA_TRUSTCert(uid, text, req, res, next) {
	try {
		  // generate a keypair
		  console.log('Generating 2048-bit RSA_DOMAIN_SUB_SUB_CA - key-pair...');
		  var keys = forge.pki.rsa.generateKeyPair(2048);
		  console.log('RSA_DOMAIN_SUB_SUB_CA Key-pair created.');
		  // create random serial no
		  var bytes = secureRandom(10, {type: 'Buffer'}) //return a Buffer of 10 bytes
		  console.log(bytes.length) //10

		  // create a certificate
		  console.log('Creating self-signed RSA_DOMAIN_SUB_SUB_CA certificate...');
		  ...
		  var attrs_issuer = [{
		      name: 'commonName',
			    ...
			    value: 'PrathamPhadnis_SUB_CA_DOMAIN Inc'
			  }, {
			    shortName: 'OU',
			    value: 'SUB_CA_DOMAIN CryptoApps'
			  }];

		  var attrs_subject = [{
			    name: 'commonName',
			    value: 'prathamesh-phadnis.com/domain/sub_subca'
			  }, {
			    name: 'countryName',
			    value: 'NO'
			  }, {
			    ...
			    name: 'organizationName',
			    value: 'PrathamPhadnis_DOMAIN_SUB_SUB_CA Inc'
			  }, {
			    shortName: 'OU',
			    value: 'DOMAIN_SUB_SUB_CA CryptoApps'
			  }];
...
//		    dataEncipherment: true
		    ...
		    altNames: [{
		      type: 6, // URI
		      value: 'https://www.prathamesh-phadnis.com/domain/sub_subca/admin'
		    }]
		  }]);
		  
		  // read domain-sub private key  
		  let pkey = fs.readFileSync('./rsa_sub_root/RSA_SUB_CA_DOMAINprvKey.pem', 'utf8');
		  ...
		  // create PKCS12
		  console.log('\nCreating DOMAIN_SUB_SUB_CA_TRUST PKCS#12...');
		  ...
		  fs.writeFile('./rsa_sub_sub_root/RSA_DOMAIN_SUB_SUB_CAcertp12b64.p12', newPkcs12Der, {encoding: 'binary'} , function (err, file) {
				if (err) throw err;
		...
		function loadPkcs12(pkcs12Der, password, caStore) {
		  ...
		  for(var sci = 0; sci < pkcs12.safeContents.length; ++sci) {
		    ...
		    for(var sbi = 0; sbi < safeContents.safeBags.length; ++sbi) {
		      ...
		      if(safeBag.type === forge.pki.oids.pkcs8ShroudedKeyBag) {
		        console.log('found DOMAIN_SUB_SUB_CA_TRUST private key');
		        map[localKeyId].privateKey = safeBag.key;
		      } else if(safeBag.type === forge.pki.oids.certBag) {
		        // this bag has a certificate
		        console.log('found DOMAIN_SUB_SUB_CA_TRUST certificate');
		  ...
		  for(var localKeyId in map) {
		    ...
		    if(entry.privateKey) {
		      ...
		      fs.writeFile('./rsa_sub_sub_root/RSA_DOMAIN_SUB_SUB_CAprvKey.pem', privateKeyP12Pem, function (err, file) {
					if (err) throw err;
		    ...
		    if(entry.certChain.length > 0) {
		      ...
		      for(var i = 0; i < certChain.length; ++i) {
		        ...
		        fs.writeFile('./rsa_sub_sub_root/RSA_DOMAIN_SUB_SUB_CACert.pem', certP12Pem, function (err, file) {
					if (err) throw err;
					console.log('Saved  DOMAIN_SUB_SUB_CA_TRUSTCert.pem file!');
				});
		      }
		      ...
		      console.log('DOMAIN_SUB_SUB_CA_TRUST Certificate chain verified: ', chainVerified);
		    }
		  }
		}
}
exports.createP12RSA_DOMAIN_SUB_SUB_CA_TRUSTCert = createP12RSA_DOMAIN_SUB_SUB_CA_TRUSTCert;
createP12RSA_DOMAIN_SUB_SUB_CA_TRUSTCert("Ajeet Phadnis", '', '', '', '');